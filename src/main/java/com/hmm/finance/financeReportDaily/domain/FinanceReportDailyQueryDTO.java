...
public class FinanceReportDailyQueryDTO 
{
	...
	public static Specification<FinanceReportDaily> getWhereClause(final FinanceReportDailyQueryDTO financeReportDailyQueryDTO) {
		return new Specification<FinanceReportDaily>() {
			...
			public Predicate toPredicate(Root<FinanceReportDaily> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
				...
				if (null!=monthDate) {
					...
					predicate.add(criteriaBuilder.lessThanOrEqualTo(root.get("date").as(Date.class),
							monthDate));
				}else {
					Date date = new Date();
					Calendar cal = Calendar.getInstance();
