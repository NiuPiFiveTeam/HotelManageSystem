...
public class WorkQueryDTO {
	...
	public static Specification<Work> getWhereClause(final WorkQueryDTO workQueryDTO ){
		return new Specification<Work>() {
			public Predicate toPredicate (Root<Work> root ,CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
				...
				if(workQueryDTO.getLackCard()!= null) {
					predicates.add(criteriaBuilder.equal(root.get("lackCard").as(Integer.class),
							workQueryDTO.getLackCard()));
				}
				
				if(workQueryDTO.getLeaveEarly() != null) {
					predicates.add(criteriaBuilder.equal(root.get("leaveEarly").as(Integer.class),
