...
public class DeptQueryDTO {
	...
	public static Specification<Department> getWhereClause(final DeptQueryDTO deptQueryDTO) {
		return new Specification<Department>() {
			...
			public Predicate toPredicate(Root<Department> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
				...
				if(StringUtils.isNoneBlank(deptQueryDTO.getID_())) {
					predicates.add(criteriaBuilder.equal(root.get("ID_").as(String.class),
							deptQueryDTO.getID_()));
				}
				...
				if(null != deptQueryDTO.getIs_parent()) {
					predicates.add(criteriaBuilder.equal(root.get("is_parent").as(String.class), 
							deptQueryDTO.getIs_parent()));
				}
