...
public class SchedulQueryDTO {
	...
	public static Specification<SchedulEvent> getWhereClause(final SchedulQueryDTO schedulQueryDTO ){
		return new Specification<SchedulEvent>() {
			public Predicate toPredicate (Root<SchedulEvent> root ,CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
				...
				if(null != schedulQueryDTO.getTraStartTime() ) {
					predicates.add(criteriaBuilder.greaterThanOrEqualTo(root.get("startDate").as(Date.class), 
							schedulQueryDTO.getTraStartTime()));
				}
				
				if( null!= schedulQueryDTO.getTraEndTime()) {
					predicates.add(criteriaBuilder.lessThanOrEqualTo(root.get("startDate").as(Date.class),
							schedulQueryDTO.getTraEndTime()));
				}
				
				if(null!= schedulQueryDTO.getEventDate() ) {
					predicates.add(criteriaBuilder.equal(root.get("eventDate").as(String.class),
							schedulQueryDTO.getEventDate()));
				}
