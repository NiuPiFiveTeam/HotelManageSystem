...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public void save(EmployeeDTO entity) {
		...
		if(entity.getIdcard()=="") {
			employ.setIdcard(null);
		}
		
		if(null!=entity.getDeptName()) {
			 department = iDeptService.findByDeptName(entity.getDeptName());
		}
		if(null != entity.getGroupName()) {
			groupRole = groupRoleService.findByGroupName(entity.getGroupName());
		}
		...
		if(null != groupRole) {
			...
			workflowService.addUser2(entity.getUserName(), entity.getPassword());
		}
		if(null != department) {
	...
	@Override
	public Page<EmployeeDTO> findAll(@Nullable Specification<Employee> spec, Pageable pageable) {
		// TODO Auto-generated method stub
					...
					if(null != groupRole) {
						...
						if(list.size()!=0) {
				    		for(GroupRole role:list) {
				    			workflowService.deleteUser(j.getUserName(), role.getGroupId());//解除关系
					    	}
				    		identityService.createMembership(j.getUserName(), groupRole.getGroupId());//创建关系
				    	}else {
				    		identityService.createMembership(j.getUserName(), groupRole.getGroupId());//创建关系
				    	}
